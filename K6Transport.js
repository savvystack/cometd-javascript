...
const Transport = cometd.Transport;
const Utils = cometd.Utils;

var K6WebSocketTransport = function(k6Socket) {
    ...
    function _websocketConnect(context) {
        ...
        _connecting = context;
        context.webSocket = k6Socket;
        k6Socket.on('open', onopen);
        k6Socket.on('close', onclose);
        k6Socket.on('error', function() {
            // Clients should call onclose(), but if they do not we do it here for safety.
            onclose({code: 1000, reason: 'Error'});
        });
        k6Socket.on('message', onmessage);
    ...
    _self.onMessage = function(context, wsMessage) {
        ...
        var close = false;
        var messages = this.convertToMessages(wsMessage);
        var messageIds = [];
    ...
    _self.accept = function(version, crossDomain, url) {
        ...
        return _webSocketSupported && _cometd.websocketEnabled !== false;
    };
...
export default K6WebSocketTransport;